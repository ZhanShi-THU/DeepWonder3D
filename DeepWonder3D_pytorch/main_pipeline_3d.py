...
from deepwonder.test_MC import run_single_view_motion_correction
...
def clear_large_variables(threshold=1 * 1024 * 1024, enabled=False):
    ...
    This helper scans the module-level global namespace, removes variables
    larger than ``threshold`` bytes based on ``sys.getsizeof``, and then
...
def main_pipeline(input_path,
                  ...
                  fast_trace_mode='raw',
                  single_view_motion_correction=False,
                  mc_max_shift=15):
    """
    ...
    segmentation (SEG), neuron merging (MN), and view merging (VM) for 3D
    datasets. The executed stages are controlled by the ``type`` string.
    ...
    Args:
        ...
        psffit_matrix_file (str): Path to the PSF fitting matrix ``.mat`` file
            (for example ``'./pth/psffit_matrix.mat'``) that encodes the
            calibration used for 3D localization.
        SR_up_rate (int or float): Upsampling factor used by the SR module,
            typically a positive integer or decimal such as 2, 4, or 8.
        ...
        single_view_motion_correction (bool, optional): If True and
            ``neuron_coords_file`` is provided, apply rigid single-view motion
            correction before the selected fast-trace mode.
        mc_max_shift (int or float, optional): Maximum absolute shift (pixels)
            used by motion correction to clip frame-wise estimates.
    ...
    Notes:
        ...
        - Runtime for each stage is stored as JSON via ``save_times_json``
          inside the ``output_dir`` hierarchy.
    """
    NOW_path = input_path
    ...
    if neuron_coords_file is not None:
        ...
        if single_view_motion_correction:
            print('FAST_TRACE mode: apply single-view motion correction before extraction...')
            run_single_view_motion_correction(
                input_path=fast_input_path,
                input_folder=fast_input_folder,
                output_path=output_dir,
                output_folder='STEP_0_MC',
                max_shift=mc_max_shift,
            )
            fast_input_path = output_dir
            fast_input_folder = 'STEP_0_MC'
        ...
        times = {
            ...
            'SINGLE_VIEW_MC': bool(single_view_motion_correction),
            'MC_MAX_SHIFT': mc_max_shift,
        }
