...
from deepwonder.DENO.DENO_buildingblocks import Encoder, Decoder, FinalConv, DoubleConv, ExtResNetBlock, SingleConv
from deepwonder.DENO.DENO_utils import create_feature_maps
...
class UNet3D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=64, layer_order='cr', num_groups=8,
        ...
        for i, out_feature_num in enumerate(f_maps_list):
            if i == 0:
                encoder = Encoder(in_channels, f_maps_en, apply_pooling=False, basic_module=DoubleConv,
                                  conv_layer_order=layer_order, num_groups=num_groups)
            else:
                encoder = Encoder(f_maps_en, f_maps_en, basic_module=DoubleConv,
                                  conv_layer_order=layer_order, num_groups=num_groups)
        ...
        for i in range(len(reversed_f_maps) - 1):
            ...
            decoder = Decoder(f_maps_de, f_maps_en, basic_module=DoubleConv,
                              conv_layer_order=layer_order, num_groups=num_groups)
        ...
        self.final_conv = nn.Conv3d(f_maps_en, out_channels, 1)
    ...
    def forward(self, x):
        ...
        paddingTime = int(np.ceil(t/8)*8-t)
        x = nn.ReplicationPad2d((0, paddingTime, 0, paddingRight, 0, paddingBottom))(x)
        # print('x -----> ',x.shape)
